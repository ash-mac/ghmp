...
const checkRoom = (room,arrivalTime,departureTime)=>{
  ...
  room.bookedSlots.forEach((slot) => {
    if(compareDates(departureTime,slot.arrivalTime)==false && compareDates(slot.departureTime,arrivalTime)==false){
      flag = false;
    }
  });
  return flag;
...
const bookRoom = expressAsyncHandler(async (req, res) =>{
    ...
    let {guestName,address,numberOfGuests,arrivalTime,departureTime,singleRoom,doubleRoom,purpose,category} = req.body;
    ...
    if(singleRoom>0 || doubleRoom>0){
      throw new Error(`We have ${singleRoom} lesser single rooms and ${doubleRoom} lesser double rooms`);
    }

    const booking = await Booking.create({guestName,address,numberOfGuests,arrivalTime,departureTime,purpose,category});
...
const removeSlotsFromRoom = async(roomId,arrivalTime,departureTime) => {
  ...
  for(let i=0;i<room.bookedSlots.length;i++){
    const slot = room[i];
    if(Date(slot.arrivalTime)===Date(arrivalTime) && Date(slot.departureTime)===Date(departureTime)){
...
const cancelBooking = expressAsyncHandler(async(req,res)=>{
  ...
  booking.roomsBooked.forEach(async(roomId)=>{
    await removeSlotsFromRoom(roomId,booking.arrivalTime,booking.departureTime);
  })
